...
public class SensorAuto extends LinearOpMode {
    ...
    public static int angleTicks(int angle)
    {
        int tickOutput;
        tickOutput = (int) (angle/18.576) * 1440;
        return tickOutput;
    }
    ...
    public void turnBot(int angle) //Negative for turn left, positive for turn rigt. Use angle to tick function.
    {
        int dist = angleTicks(angle); //Converts angles to tick for wheels
        dist = dist / 2; //gearing compensation
        frontLeftDrive.setTargetPosition(dist);
        frontRightDrive.setTargetPosition(-dist);
        backLeftDrive.setTargetPosition(dist);
        backRightDrive.setTargetPosition(-dist);
        positionRun();
    } //NOTE: This differs from strafe and goforward as it executes movement in method, not separate from.
    ...
    {
        while (returnColor(150) != targetColor && opModeIsActive())
        {
            encoderReset();
            ...
            if (tfod != null) {
                ...
                if (updatedRecognitions != null) {
                    ...
                    if (quadCounter == 0)
                    {
                        ...
                        quadCounter++;
                        encoderReset();
                        moveToColor(622,"blue");
                    }
                    if (updatedRecognitions.size() != 0){
                        switch (updatedRecognitions.get(0).getLabel())
                        {
                            case "Single":
                                if (singleCounter > 50)
                                {
